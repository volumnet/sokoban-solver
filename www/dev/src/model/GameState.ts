...
export default class GameState {
  ...
  checkWall(initial: Point, checkDirection: BlockMovement): Point[] {
    ...
    do {
      ...
      if (this.winState.hasPoint(next)) {
        // Это выигрышная позиция, не считаем за стену
        // console.log(next.str + ' is win');
        return [];
      }
      if (possibleWall && !this.room.hasPoint(possibleWall)) {
        // Нет стены с обозначенного направления, вся стена аннулируется
        // console.log(next.str + ' has not wall aside ' + possibleWall.str);
        return [];
      }
      if (this.room.hasPoint(next)) {
        // Текущая позиция - стена, прерываем цикл с текущим результатом
  ...
  nextStates(restrictToBox: Point | null = null): { [key: string]: GameState } {
    ...
    for (let box of boxesSet) {
      for (let move of [
        ...
        if (oppositeMove == BlockMovement.None) {
          // Не задано движение (для совместимости)
          continue;
        }
        oppositeSide = box.stepTo(oppositeMove);
        if (!oppositeSide) {
          // Не доступна противоположная движению сторона
          continue;
        }
        const oppositeSidePath = canReachTo[oppositeSide.toString()];
        if (oppositeSidePath === undefined) {
          // Не может пройти до противоположной стороны; undefined, поскольку первая точка со значением ''
          continue;
        }
        if (!this.blockMovementAvailable(box, move)) {
          // Не получится сдвинуть блок в эту сторону
          continue;
        }
  ...
  blockMovementAvailable(block: Point, move: BlockMovement): boolean {
    ...
    if (this.isEngaged(newPoint)) {
      // Место занято
      return false;
    }
    if (this.winState.hasPoint(newPoint)) {
      // Выигрышное место
      return true;
    }
    if (this.room.corners.hasPoint(newPoint)) {
      // Угол стены, но не выигрышная позиция
      return false;
    }
    if (this.nearWalls.hasPoint(newPoint)) {
      // Позиция возле стены (не выигрышная, это определяется при определении стен)
      return false;
    }
  ...
  playMove(move: BlockMovement): GameState | null | never {
    ...
    if (this.boxes.hasPoint(nextPoint)) {
      // Двигаем ящик
